...
export const ApplicationService = {
  ...
  async getById(id: string) {
    const app = await ApplicationModel.findById(id).populate('job').populate('applicant');
  ...
  async findAll(query: {
    ...
    const q = ApplicationModel.find(filter).populate('job').populate('applicant').skip(skip).limit(limit).sort({ createdAt: -1 });

    const [total, items] = await Promise.all([ApplicationModel.countDocuments(filter), q.exec()]);

    return { total, page, limit, items };
  },

  async update(id: string, payload: Partial<{ coverLetter?: string; resumeUrl?: string; notes?: string }>) {
    const app = await ApplicationModel.findByIdAndUpdate(id, payload as any, { new: true }).populate('job').populate('applicant');
    if (!app) throw new ApiError('Application not found', 404);
    return app;
  },
  ...
  async updateStatus(id: string, status: string, notes?: string) {
    ...
    // Basic transition checks could be added here
  ...
  async withdraw(id: string) {
    ...
    // mark as withdrawn (soft delete) for auditability
